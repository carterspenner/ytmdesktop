...
if (process.platform !== "linux" && !app.isDefaultProtocolClient("ytmd")) {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      log.info("Application set as default protocol client for 'ytmd'");
...
function shouldDisableUpdates(): boolean {
...
const createOrShowSettingsWindow = (): void => {
  ...
  settingsWindow.webContents.setWindowOpenHandler(details => {
    if (
      details.url === "https://github.com/snowdriftdev/ytmdesktop" ||
...
app.on("ready", async () => {
  ...
  session.fromPartition(app.isPackaged ? "persist:ytmview" : "persist:ytmview-dev").setPermissionCheckHandler((webContents, permission) => {
    if (ytmView && webContents === ytmView.webContents) {
